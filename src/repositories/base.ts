...
import { RequireAtLeastOne, StringKeys } from '../utils/types';
...
export abstract class BaseRepository<T extends Model = Model> {
    ...
    create(payload: CreationAttributes<T>, options?: any): Promise<T> {
        return this.model.create(payload, options) as Promise<T>;
    }
    ...
    findManyAndCount(where: WhereOptions<T>, options: FindManyOptions<T> = {}) {
        const { limit, offset, order, attributes } =
            this.getSortingAndPaginationOptions(options);
    ...
    async findAll(where: WhereOptions<T>, options: FindOptions<InferAttributes<T>> = {}) {
